...
namespace dd::nixrar {
MainWindow::MainWindow(QStringList args, QWidget *parent, Qt::WindowFlags flags) :
    ...
    connect(this->ui->actionCopy_Full_Names_to_Clipboard, SIGNAL(triggered()), this, SLOT(copyFullNamesToClipboard()));
    ...
    connect(this->ui->actionExtract_to_Specified_Folder, SIGNAL(triggered()), this, SLOT(extractToSpecifiedFolder()));
    ...
    connect(this->ui->actionExtract_Without_Confirmation, SIGNAL(triggered()), this, SLOT(extractWithoutConfirmation()));
    connect(this->ui->actionProtect_Archive_from_Damage, SIGNAL(triggered()), this, SLOT(protectArchiveFromDamage()));
...
void MainWindow::addDroppedFiles(const QStringList &filePaths) {
    ...
    QProgressDialog progressDialog(tr("Adding files to archive..."), tr("Cancel"), 0, static_cast<int>(filePaths.size()), this);
    ...
    progressDialog.setValue(static_cast<int>(filePaths.size())); //FIXME: This sets the progress bar value to the length of the list, need to move this up in the for loop
...
void MainWindow::saveArchiveCopyAs() {
    ...
    const auto fileName = QFileDialog::getSaveFileName(this,
         tr("Save Archive"), QDir::homePath(), tr("Archive Files (*.rar *.zip *.cab *.arj *.lzh *.ace *.tar *.gzip *.uue *.bz2 *.tar *.gz)", "*.zip")); //TODO Set the default file extension to the archive type
...
void MainWindow::copyFullNamesToClipboard() {
}
